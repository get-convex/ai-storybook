...
import { ChatMessagePromptTemplate, ChatPromptTemplate, HumanMessagePromptTemplate, SystemMessagePromptTemplate } from "langchain/prompts";
import { SequentialChain } from "langchain/chains";

export const populatePageImage = action(
    async ({runQuery, runMutation}, {pageNumber, version}: {pageNumber: number, version: number}) => {
    console.log(
        `Hello from populatePageImage for page ${pageNumber} at book version ${version}`);
    const [currentVersion, book] = 
        await runQuery("chapters:getBookStateWithVersion");
...
async function getPageImage(book: Doc<"chapters">[], pageNumber: number):
...
function getImageChain(): LLMChain {
    const promptTemplate = new PromptTemplate({
        template: "{pageDescription} in the style of a children's book illustration",
        inputVariables: ["pageDescription"],
    });
    const imageModel = new Replicate({
        model: "ai-forever/kandinsky-2:601eea49d49003e6ea75a11527209c4f510a93e2112c969d548fbb45b9c4f19f",
